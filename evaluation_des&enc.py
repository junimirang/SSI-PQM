...
def run_ntru_operations(security_levels, iterations=1):
    ...
    for level in security_levels:
        ...
        # 암호화
        encrypt_cmd = f"python3 NTRU.py -k NTRU_key_level{level} -eF message1.txt -O enc_level{level}.dat"
        subprocess.run(encrypt_cmd, shell=True, input=message.encode())

        # 암호화 성능 측정 종료
        end_time = time.time()
        enc_time_usage = end_time - start_time
        print(f"LEVEL: {level} => Time for Encryption: {enc_time_usage:.4f} seconds")
        ...
        dec_memory_usage = end_memory - start_memory
        ...
        if decrypted_message == message:
            # sprint(f"Decryption successful at security level {level}.")
            print(f"LEVEL: {level} => Time for Decryption: {dec_time_usage:.4f} seconds")
        else:
